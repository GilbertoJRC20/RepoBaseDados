...
insert into Receitas values
('2004-04-02','19:11:00','71','Alexandre Mota','Doces Variados','Os doces são feitos sempre com...');

insert into Receitas values
('2004-04-02','19:11:00','56','Paulo Perdiz','doces Baunilha','Os doces são feitos sempre com...');

select * from Receitas;

--5 a)
create view ReceitasChoco as
select count(titulo) as contagem
from Receitas
where descricao like '%chocolate%' or titulo like '%chocolate%'

select * from ReceitasChoco;

--6 e) quantas receitas têm um dado ingrediente
create procedure ReceitasIngre (@ingre varchar(MAX)) as
select count(titulo) as contagem
from Receitas
where descricao like concat('%',@ingre,'%') or titulo like concat('%',@ingre,'%')
go

exec ReceitasIngre @ingre='Doces'

--6 a)
create procedure ReceitasChefe (@chefe int) as
select count(chefe) as contagem
from receitas
where chefe=@chefe
go

exec ReceitasChefe @chefe=81
